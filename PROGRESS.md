...
- [x] **Phase 11 — On-screen reply captions (2026-07-04)**: the `set_emotion` tool gained a required `text` argument — a short caption of the reply (max ~6 words, in the reply's language) written by the model itself. `AI_SYSTEM_PROMPT` updated accordingly; both engine clients declare/parse the new argument and the `onEmotion` callback now carries `(emotion, text)`. `displayShowFace` gained an optional caption mode (smaller face, caption word-wrapped up to 3 lines with `..` truncation, state text demoted to a small footer); new `appSetTurnFace`/`appClearTurnFace` in `AppState.cpp` keep the model's face + caption on screen through the THINKING→SPEAKING repaints (previously the first TTS chunk wiped the emotion face with the generic "Speaking..." one). The turn face is cleared on barge-in, button abort, disconnect, and the listening/thinking timeouts so a stale caption can't leak into the next turn. **RESULT state (same date)**: auto-reopening the mic after each reply (continuous conversation) was replaced by push-to-talk rounds with a dedicated `AssistantState::RESULT` — `onTurnComplete` transitions to RESULT (dim-purple LED), whose screen is the reply's face + caption with a "BOOT/PLAY = talk" footer (happy face fallback if the model skipped set_emotion); the talk button goes RESULT → LISTENING for the next round. The held response survives WS drops/reconnects (e.g. Gemini `goAway` session expiry): `onDisconnected`/`onReady` leave the RESULT screen alone and only update the WS dot. A `turnComplete` arriving after a button-abort (state already IDLE) is ignored so it can't resurrect a dead turn's screen. **Fix — RESULT was reachable in theory but bypassed in practice (same date)**: three paths skipped or destroyed the RESULT hold and were closed: (1) `onInterrupted` unconditionally jumped back to LISTENING while `s_conversationActive` was still true — since the mic only streams during LISTENING, an interrupt during THINKING/SPEAKING can only be stale/noise audio (this board's mic is flaky), so it now holds RESULT (or IDLE if no reply face yet) instead; (2) a WS drop mid-reply (big TTS frames can starve the heartbeat) ate the `turnComplete` and wiped to IDLE → "Connected!" — `onDisconnected` now holds RESULT when a reply face exists; (3) a trailing audio chunk after `turnComplete` flipped RESULT back to SPEAKING with nothing to ever end that phantom turn — `onAudio` now only enters SPEAKING from LISTENING/THINKING. Also added `[State] X -> Y` serial logging on every transition (`appStateName`, reused by the MENU info screen). Build-verified 2026-07-04 for both `AI_ENGINE_GEMINI` and `AI_ENGINE_OPENAI`; **not yet verified against the live APIs on hardware**.
